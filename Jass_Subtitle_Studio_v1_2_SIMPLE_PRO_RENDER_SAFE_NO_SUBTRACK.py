...
from PySide6.QtCore import Qt, QUrl, QTimer, Signal, QSize, QProcess
...
class VideoPreview(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.text = ""
        self.values = {
            "font": "Arial", "size": 24, "color": "#ffffff",
...
class MainWindow(QMainWindow):
    ...
    def choose_video(self):
        ...
        # The selected file is ALWAYS the immutable source.  Selecting a new
        # video also resets the output field so an old output path can never
        # accidentally become the new source file.
        source = Path(path).resolve()
        self.video_path = str(source)

        self.player.setSource(QUrl.fromLocalFile(str(source)))

        # Always generate a fresh, separate output name for the selected
        # source.  Never reuse the previous video's output path.
        candidate = source.with_name(source.stem + "_subtitled.mp4")
        counter = 2
        while candidate.resolve() == source or candidate.exists():
            candidate = source.with_name(
                f"{source.stem}_subtitled_{counter}.mp4"
            )
            counter += 1

        self.output_path = str(candidate)
        self.output.setText(self.output_path)
        self.status.setText(f"Loaded video: {source.name}")
...
"""
    ...
    def create_video(self):
        """Burn subtitles into a NEW output file; never modify the source."""
        if self.render_process is not None:
            QMessageBox.information(
                self, "Rendering", "A render is already running."
            )
            return

        if not self.video_path:
            QMessageBox.information(
                self, "Choose video", "Choose a video first."
            )
            return

        if not self.entries:
            QMessageBox.information(
                self, "Add subtitles", "Load an SRT or TXT file first."
            )
            return
        ...
        if not self.ffmpeg:
            QMessageBox.warning(
                self,
                "FFmpeg required",
                "Choose ffmpeg.exe with the FFmpeg… button, "
                "or install FFmpeg and add it to PATH.",
            )
            return

        # Capture immutable source/output paths for THIS render.  Nothing in
        # the render path ever assigns to self.video_path.
        source = Path(self.video_path).resolve()
        output = Path(self.output_path).resolve() if self.output_path else (
            source.with_name(source.stem + "_subtitled.mp4")
        )

        if not source.exists() or not source.is_file():
            QMessageBox.critical(
                self, "Source video", f"Source video was not found:\n\n{source}"
            )
            return

        # Strong protection against accidental in-place rendering.  resolve()
        # catches normal path/symlink aliases; samefile() catches hard links
        # when the output already exists.
        try:
            same_file = output.exists() and os.path.samefile(source, output)
        except OSError:
            same_file = False
        ...
        if output == source or same_file:
            QMessageBox.warning(
                self,
                "Invalid output",
                "The output video must be different from the source video.\n\n"
                "The original video is protected and will not be overwritten.",
            )
            return

        if output.parent == source.parent and output.name.lower() == source.name.lower():
            QMessageBox.warning(
                self,
                "Invalid output",
                "The output filename is the same as the source filename. "
        ...
        # Do not render directly into the final output.  FFmpeg writes to a
        # temporary file first; only a successful render is promoted to the
        # requested output.  This protects the existing output from a partial
        # render as well as protecting the source video.
        temp_output = output.with_name(
            f".{output.stem}.jass_rendering{output.suffix or '.mp4'}"
        )

        if temp_output.resolve() == source or (
            temp_output.exists() and os.path.samefile(source, temp_output)
        ):
            QMessageBox.critical(
                self, "Render protection", "Temporary output would overwrite the source."
            )
            return
        ...
        try:
            ...
            self._render_source = str(source)
            self._render_ass = ass
            self._render_temp = str(temp_output)
            self._render_out = str(output)
            self._render_buffer = ""
            self._render_cancelled = False

            self.render.setEnabled(False)
            self.quick_render.setEnabled(False)
            self.cancel.setEnabled(True)
            self.progress.setValue(0)
            self.status.setText("Rendering video… Original video is protected.")

            process = QProcess(self)
            process.setProcessChannelMode(QProcess.MergedChannels)
            process.readyReadStandardOutput.connect(self.on_render_output)
            process.finished.connect(self.finish_render)
            process.errorOccurred.connect(self.render_process_error)
            self.render_process = process

            process.start(self.ffmpeg, args)
            ...
            self.quick_render.setEnabled(True)
            self.cancel.setEnabled(False)
            self.render_process = None
            QMessageBox.critical(self, "Render error", str(e))

    def on_render_output(self):
        """Drain FFmpeg progress output without blocking the GUI."""
        ...
        # QProcess may deliver several lines at once or split a line across
        # reads, so retain the incomplete tail for the next signal.
        lines = self._render_buffer.split("\n")
        self._render_buffer = lines.pop()
        ...
        for raw_line in lines:
            ...
            if line.startswith("out_time_ms="):
                ...
                if self.duration > 0:
                    percent = int(elapsed_ms * 100 / self.duration)
                    self.progress.setValue(max(0, min(99, percent)))

    def finish_render(self, exit_code, exit_status):
        """Promote a successful temporary render to the final output."""
        ass = getattr(self, "_render_ass", None)
        temp = getattr(self, "_render_temp", None)
        out = getattr(self, "_render_out", None)
        source = getattr(self, "_render_source", None)
        cancelled = getattr(self, "_render_cancelled", False)
        ...
        if cancelled:
            ...
            self.progress.setValue(0)
            self.status.setText("Rendering cancelled. Original video unchanged.")
            return
        ...
        succeeded = (
            ...
            and Path(temp).stat().st_size > 0
            and out is not None
        )

        if succeeded:
            # Final safety check immediately before promotion.
            source_path = Path(source).resolve() if source else None
            output_path = Path(out).resolve()
            try:
                if output_path.exists() and source_path and os.path.samefile(source_path, output_path):
                    Path(temp).unlink(missing_ok=True)
                    raise RuntimeError("Safety check blocked an attempt to overwrite the source video.")
            except FileNotFoundError:
                pass

            try:
                # Replace only the requested OUTPUT. The source is never touched.
                os.replace(temp, output_path)
            except Exception as exc:
                Path(temp).unlink(missing_ok=True)
                self.progress.setValue(0)
                self.status.setText("Could not finalize rendered video.")
                QMessageBox.critical(
                    self, "Output error", f"Could not finalize the output video:\n\n{exc}"
                )
                return
            ...
            if self.copy_srt.isChecked():
                output_path.with_suffix(".srt").write_text(
                    write_srt(self.entries), encoding="utf-8"
                )

            self.status.setText(
                f"✓ Created: {output_path.name}  •  Source unchanged"
            )
            QMessageBox.information(
                self,
                "Finished",
                f"Subtitled video created:\n\n{output_path}\n\n"
        ...
        self.progress.setValue(0)
        self.status.setText("Rendering failed. Original video unchanged.")
        QMessageBox.critical(
            self,
            "FFmpeg error",
            "FFmpeg could not create the video. The original video was not modified.",
        )

    def render_process_error(self, error):
        """Report process-start/runtime errors from QProcess."""
        if error == QProcess.FailedToStart:
            ...
            if ass:
                Path(ass).unlink(missing_ok=True)
            self.render_process = None
            self.status.setText("FFmpeg could not be started. Original video unchanged.")
            QMessageBox.critical(
                self,
                "FFmpeg error",
    ...
    def cancel_render(self):
        ...
        if process.state() != QProcess.NotRunning:
            ...
            self.status.setText("Cancelling rendering…")
            process.terminate()

            # Give FFmpeg a short opportunity to exit cleanly. If it does not,
            # force termination so the UI can never remain stuck indefinitely.
            QTimer.singleShot(2000, self._force_cancel_render)

    def _force_cancel_render(self):
        process = self.render_process
